...
namespace Dal_Planilha
{
    public class resultadoDal
    {
        ...
        public void AlterarPropTabResultado(int Cod, int Ano, decimal tabProposito)
        {
            //Altera os dados no DB
            MySqlCommand cmd = new MySqlCommand("UPDATE " + Cod + "tbl_total SET pl_propostaTabResultado = @tabProposito WHERE pl_ano = @Ano; ");
            cmd.Parameters.Add("@Ano", MySqlDbType.Int32).Value = Ano;
            cmd.Parameters.Add("@tabProposito", MySqlDbType.Decimal).Value = tabProposito;
        ...
        public void AlterarRealiTabResultado(int Cod, int Ano, decimal tabRealizado)
        {
            //Altera os dados no DB
            MySqlCommand cmd = new MySqlCommand("UPDATE " + Cod + "tbl_total SET pl_realizadoTabResultado = @tabRealizado WHERE pl_ano = @Ano; ");
            cmd.Parameters.Add("@Ano", MySqlDbType.Int32).Value = Ano;
            cmd.Parameters.Add("@tabRealizado", MySqlDbType.Decimal).Value = tabRealizado;
        ...
        public DataTable MetaRealizada(int Ano, int Cod)
        {
            string queryString = "SELECT sum(pl_realizadoTabResultado / pl_realiResultado * 100 ) as Meta_Realizada FROM " + Cod + "tbl_total tot " +
                "INNER JOIN " + Cod + "tbl_resultado res ON tot.pl_ano = res.pl_ano " +
                "WHERE tot.pl_ano = @Ano AND res.cod_categoria = 1; ";
